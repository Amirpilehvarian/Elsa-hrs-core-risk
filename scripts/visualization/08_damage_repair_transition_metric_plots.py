#!/usr/bin/env python3
"""Plot transition-level mean +/- SE metrics for damage and repair analyses."""
...
def summarize_panel(df: pd.DataFrame, metric: str, panel: str) -> pd.DataFrame:
    ...
    work = work.dropna(subset=[metric, "transition", "model"])
    if panel != "all":
    ...
    summary = (
        work.groupby(["model", "transition"], as_index=False)
        .agg(
...
def approx_target_count(panel: pd.DataFrame) -> str:
    if panel.empty or "target_count" not in panel.columns:
        return ""
    approx = int(round(panel["target_count"].mean()))
...
def plot_metric(
    ...
    fig.suptitle(f"{dataset_label} | {run_tag} | {event_type.upper()} | {spec['label']} (LR vs DNN)", fontsize=18, y=0.98)
    ...
    for idx, panel_name in enumerate(PANEL_ORDER):
        ...
        for model in MODEL_ORDER:
            ...
            sub = panel[panel["model"] == model].set_index("transition").reindex(transitions)
        ...
        ax.text(0.03, 0.92, approx_target_count(panel), transform=ax.transAxes, fontsize=11)
        ax.set_xticks(x)
...
def run_event_family(metrics_root: Path, out_root: Path, run_tag: str, dataset_label: str, event_type: str) -> None:
    ...
    df = df[df["event_type"].eq(event_type)].copy()

    for metric in METRIC_SPECS:
        plot_metric(
            df=df,
            metric=metric,
            out_path=out_root / run_tag / event_type / METRIC_SPECS[metric]["filename"],
...
def main() -> None:
    ...
    ap.add_argument("--dataset-label", default=None, help="Dataset label used in titles")
    ap.add_argument("--metrics-root", default="analysis/ELSA/metrics/damage_repair")
    ap.add_argument("--out-root", default="analysis/ELSA/figures/damage_repair_transition_metrics")
    args = ap.parse_args()
    ...
    for event_type in ("damage", "repair"):
        run_event_family(metrics_root=metrics_root, out_root=out_root, run_tag=run_tag, dataset_label=dataset_label, event_type=event_type)
