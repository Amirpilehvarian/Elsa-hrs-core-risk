#!/usr/bin/env python3
"""Make pooled calibration plots stratified by sex using frozen OOF probabilities."""
...
def pooled_truth_probs_by_sex(
    ...
    ys, ps = [], []
    for trans_fp in sorted(trans_dir.glob("S*_S*.csv"), key=lambda p: infer_transition(p) or (999, 999)):
        parsed = infer_transition(trans_fp)
        if parsed is None:
            continue
        _, tp1 = parsed
        trans = trans_fp.stem
        probs_fp = probs_dir / f"probs_{trans}.csv"
...
def main() -> None:
    ...
    target_sets = {
        "pooled_all": DISEASE_TARGETS + ADL_TARGETS,
    ...
    for sex in SEX_ORDER:
        for label, targets in target_sets.items():
            ...
            title = f"{dataset_label} | {run_tag} | {sex.title()} | {label.replace('_', ' ')}"
            out_fp = Path(args.out_root) / run_tag / sex / f"{label}.png"
