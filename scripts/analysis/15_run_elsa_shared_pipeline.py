#!/usr/bin/env python3
"""Run the strict ELSA shared-subset pipeline used for HRS-matched validation."""
...
def main() -> None:
    ...
    ap.add_argument("--run-tag", default="ELSA_SHARED")
    ap.add_argument("--mapping-csv", default="data/schema/harmonized/elsa_hrs_shared_mapping.csv")
    ap.add_argument("--source-dir", default="Transition_data/01_transitions/MAR_Cart")
    ...
    if not args.skip_transitions:
        run_step(
            "Build ELSA Shared Transitions",
            [
                sys.executable,
                "scripts/analysis/11_build_elsa_shared_transitions.py",
                "--mapping-csv",
                args.mapping_csv,
                "--source-dir",
                args.source_dir,
    ...
    if not args.skip_models:
        ...
        run_step("Fit ELSA Shared DNN", dnn_cmd, root, args.dry_run)

    if not args.skip_metrics:
        for model in ("LR", "DNN"):
            run_step(
                f"Evaluate ELSA Shared {model}",
                [
                    sys.executable,
    ...
    if not args.skip_hazard:
        run_step(
            "Build ELSA Shared Hazard Ratio Tables",
            [
                sys.executable,
    ...
    if not args.skip_calibration_plots:
        ...
        if args.include_loglog_calibration:
            run_step(
                "Plot ELSA Shared Calibration (Log-Log)",
                [
                    sys.executable,
                    "scripts/visualization/01_plot_calibration.py",
                    "--run-tag",
    ...
    if not args.skip_hazard_plots:
        run_step(
            "Plot ELSA Shared Hazard Ratios",
            [
                sys.executable,
    ...
    if not args.skip_ece_plots:
        run_step(
            "Plot ELSA Shared ECE Comparisons",
            [
                sys.executable,
    ...
    if args.include_damage_repair:
        for model in ("LR", "DNN"):
            run_step(
                f"Build ELSA Shared Damage Repair Events {model}",
                [
                    sys.executable,
                    ...
                    args.analysis_root,
                    "--out-root",
                    str(Path(args.derived_root) / "damage_repair"),
                ],
                root,
                args.dry_run,
            )
            run_step(
                f"Evaluate ELSA Shared Damage Repair {model}",
                [
                    sys.executable,
        ...
        run_step(
            "Plot ELSA Shared Damage Repair Calibration",
            [
                sys.executable,
        ...
        if args.include_loglog_calibration:
            run_step(
                "Plot ELSA Shared Damage Repair Calibration (Log-Log)",
                [
                    sys.executable,
                    "scripts/visualization/05_damage_repair_calibration_plots.py",
                    "--run-tag",
    ...
    if not args.skip_metric_summary_plots:
        ...
        run_step("Plot ELSA Shared Target-Level Metric Summaries", metric_summary_cmd, root, args.dry_run)

    print("\n[DONE] ELSA shared-subset pipeline complete.")
